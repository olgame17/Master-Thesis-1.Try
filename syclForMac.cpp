...
void vecAddRunAKernelOnOneDevice(int size){
    ...
    { // By sticking all the SYCL work in a {} block, we ensure
        ...
        gpu_selector device_selector_host;
        
        // Create a queue to work on
        queue myQueue(device_selector_host);
        
        std::cout << "Is CPU: "
        << myQueue.get_device().is_cpu()
        << ", Is GPU: " << myQueue.get_device().is_gpu()
        << ", Is Host: " << myQueue.get_device().is_host()
        << ", Is Accelerator: " << myQueue.get_device().is_accelerator()
        << "\n";
        ...
        myQueue.submit([&](handler &cgh) {
            ...
            int workItemNo=size;
            cgh.parallel_for<class vec_add>(range<1>(workItemNo),[=](item<1> myItem) {
...
//        Bit-Compression
//*****************************
void bitCompressionPrintTheResult(uint4 *input, uint * num_bits, uint * C, int size){
    //print out the result (on the host side)
...
void matMulRunAKernelOnOneDevice(int size){
    ...
    int sqrtofthesize=std::sqrt(size);
    int width = (int)std::floor(sqrtofthesize);
