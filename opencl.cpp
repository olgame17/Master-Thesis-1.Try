...
void vecAddCopyInputHostArrayToDeviceArray(int * A,int * B,cl::Buffer buffer_A,cl::Buffer buffer_B,cl::CommandQueue queue, int size, int offset){
    //write arrays A and B to the device
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(int)*size,A);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(int)*size,B);
}
...
void vecAddRunTheKernel(cl::Kernel kernel_add,cl::Program program,
   ...
   // queue.enqueueNDRangeKernel(kernel_add,cl::NullRange,cl::NDRange(size),cl::NullRange);
    queue.enqueueNDRangeKernel(kernel_add,offset,cl::NDRange(size),cl::NullRange);
    queue.finish();
}

void vecAddCopyOutputDeviceArrayToHostArray(int * C,cl::Buffer buffer_C,
                                            cl::CommandQueue queue,int size, int offset){
    //read result C from the device to array C
    queue.enqueueReadBuffer(buffer_C,CL_TRUE,offset,sizeof(int)*size,C);
    
}

void vecAddPrintTheResult(int *A, int *B, int * C, int size){
...
void bitCompressionCopyInputHostArrayToDeviceArray(cl_uint4 * input,cl_uint * num_bits,cl::Buffer buffer_A,cl::Buffer buffer_B,cl::CommandQueue queue, int size, int offset){
    //write arrays input and num_bits to the device
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(cl_uint4)*size,input);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(cl_uint)*size,num_bits);
}
...
void bitCompressionRunTheKernel(cl::Kernel bit_compression,cl::Program program,
    ...
    queue.enqueueNDRangeKernel(bit_compression,offset,cl::NDRange(size),cl::NullRange);
    queue.finish();
}
...
void bitCompressionCopyOutputDeviceArrayToHostArray(cl_uint * C,cl::Buffer buffer_C,
                                            cl::CommandQueue queue,int size, int offset){
    //read result C from the device to array C
    queue.enqueueReadBuffer(buffer_C,CL_TRUE,offset,sizeof(cl_uint)*size,C);
    
}

void bitCompressionPrintTheResult(cl_uint4 *input, cl_uint * num_bits,
                                  cl_uint * C, int size){
    //print out the result (on the host side)
...
void bitCompressionPrintTheResultForTwoDevices(cl_uint4 *input1, cl_uint4 *input2,
                                               cl_uint * num_bits1, cl_uint * num_bits2,
                                               cl_uint *output1, cl_uint *output2,
                                               int halfOfTheSize){
    //print out the result (on the host side)
    printf("======================\n= Bit Compression Done\n");
    std::cout<<" result: \n";
    ...
    unsigned int check1 = 1, check2 =1 ;
    int c1 = 0, c2 =0;
    for(unsigned int i = 0; i < halfOfTheSize; ++i) {
        cl_uint4 in1 = input1[i];
        int bits1= num_bits1[i];
        cl_uint tmp1=0;
        if (bits1 == 2) {
            tmp1 |= (in1.x << (32-bits1)) & 3221225472u;
            tmp1 |= (in1.y << (28-bits1)) &  805306368u;
            tmp1 |= (in1.z << (24-bits1)) &  201326592u;
            tmp1 |= (in1.w << (20-bits1)) &   50331648u;
        } else if (bits1 == 4) {
            tmp1 |= (in1.x << (32-bits1)) & 4026531840u;
            tmp1 |= (in1.y << (28-bits1)) &  251658240u;
            tmp1 |= (in1.z << (24-bits1)) &   15728640u;
            tmp1 |= (in1.w << (20-bits1)) &     983040u;
        } else if (bits1 == 8) {
            tmp1 |= (in1.x << (32-bits1)) & 4278190080u;
            tmp1 |= (in1.y << (28-bits1)) &   16711680u;
            tmp1 |= (in1.z << (24-bits1)) &      65280u;
            tmp1 |= (in1.w << (20-bits1)) &        255u;
        }
        c1 = tmp1;
        ...
        cl_uint4 in2 = input2[i];
        int bits2= num_bits2[i];
        cl_uint tmp2=0;
        if (bits2 == 2) {
            tmp2 |= (in2.x << (32-bits2)) & 3221225472u;
            tmp2 |= (in2.y << (28-bits2)) &  805306368u;
            tmp2 |= (in2.z << (24-bits2)) &  201326592u;
            tmp2 |= (in2.w << (20-bits2)) &   50331648u;
        } else if (bits2 == 4) {
            tmp2 |= (in2.x << (32-bits2)) & 4026531840u;
            tmp2 |= (in2.y << (28-bits2)) &  251658240u;
            tmp2 |= (in2.z << (24-bits2)) &   15728640u;
            tmp2 |= (in2.w << (20-bits2)) &     983040u;
        } else if (bits2 == 8) {
            tmp2 |= (in2.x << (32-bits2)) & 4278190080u;
            tmp2 |= (in2.y << (28-bits2)) &   16711680u;
            tmp2 |= (in2.z << (24-bits2)) &      65280u;
            tmp2 |= (in2.w << (20-bits2)) &        255u;
        }
        c2 = tmp2;
...
void matMulCopyInputHostArrayToDeviceArray(int * input1,int * input2,cl::Buffer buffer_A,cl::Buffer buffer_B,cl::CommandQueue queue, int size, int offset){
    //write arrays input and num_bits to the device
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(int)*size,input1);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(int)*size,input2);
}
...
void matMulRunTheKernel(cl::Kernel mat_mul,cl::Program program,
    ...
    queue.enqueueNDRangeKernel(mat_mul,offset,cl::NDRange(size),cl::NullRange);
    queue.finish();
}
...
void linRegCopyInputHostArrayToDeviceArray(float * input1, float * input2,
    ...
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(float)*size,input1);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(float)*size,input2);
    queue.enqueueWriteBuffer(buffer_C,CL_TRUE,offset,sizeof(float)*size,alpha);
    queue.enqueueWriteBuffer(buffer_D,CL_TRUE,offset,sizeof(float)*size,beta);
}
...
void linRegRunTheKernel(cl::Kernel lin_reg,cl::Program program,
                        cl::Buffer buffer_A,cl::Buffer buffer_B,
                        cl::Buffer buffer_C,cl::Buffer buffer_D,
                        cl::Buffer buffer_E,cl::CommandQueue queue,int size, int offset){
    ...
    queue.enqueueNDRangeKernel(lin_reg,offset,cl::NDRange(size),cl::NullRange);
    queue.finish();
}
...
void linRegCopyOutputDeviceArrayToHostArray(float * output,cl::Buffer buffer_E,
                                            cl::CommandQueue queue,int size, int offset){
    //read result C from the device to array C
    queue.enqueueReadBuffer(buffer_E,CL_TRUE,offset,sizeof(float)*size,output);
...
void syr2kCopyInputHostArrayToDeviceArray(float * input_a, float * input_b,
                                                ...
                                                cl::CommandQueue queue, int newSize, int offset){
    //write arrays input_a,input_b,output to the device
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(float)*newSize,input_a);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(float)*newSize,input_b);
    queue.enqueueWriteBuffer(buffer_C,CL_TRUE,offset,sizeof(float)*newSize,output);
}

void syr2kRunTheKernel(cl::Kernel syr2k ,cl::Program program,
                             cl::Buffer buffer_A,cl::Buffer buffer_B,
                             cl::Buffer buffer_C, float ALPHA, float BETA,
                            int M, int N, cl::CommandQueue queue,int newSize, int offset){
    ...
    syr2k.setArg(5,M);
    syr2k.setArg(6,N);
    queue.enqueueNDRangeKernel(syr2k,offset,cl::NDRange(newSize),cl::NullRange);
    queue.finish();
}
...
void syr2kCopyOutputDeviceArrayToHostArray(float * output,cl::Buffer buffer_C,
                                                 cl::CommandQueue queue,int newSize, int offset){
    //read result C from the device to array C
    queue.enqueueReadBuffer(buffer_C,CL_TRUE,offset,sizeof(float)*newSize,output);
}
...
void convolutionCopyInputHostArrayToDeviceArray(int * input, int * mask,
    ...
    //write arrays input_a,input_b,output to the device
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,offset,sizeof(int)*newSize,input);
    queue.enqueueWriteBuffer(buffer_B,CL_TRUE,offset,sizeof(int)*mask_size,mask);
    queue.enqueueWriteBuffer(buffer_C,CL_TRUE,offset,sizeof(int)*newSize,output);
}
...
void convolutionRunTheKernel(cl::Kernel convolution ,cl::Program program,
    ...
    queue.enqueueNDRangeKernel(convolution,offset,cl::NDRange(newSize),cl::NullRange);
    queue.finish();
}
...
void convolutionCopyOutputDeviceArrayToHostArray(int * output,cl::Buffer buffer_C,
                                           cl::CommandQueue queue,int newSize, int offset){
    //read result C from the device to array C
    queue.enqueueReadBuffer(buffer_C,CL_TRUE,offset,sizeof(int)*newSize,output);
}
...
void vecAddRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    vecAddCopyInputHostArrayToDeviceArray(A,B,buffer_A,buffer_B,queue,size,0);
    
    cl::Kernel kernel_add=cl::Kernel(program,"vec_add");
    vecAddRunTheKernel(kernel_add,program,buffer_A,buffer_B,buffer_C,queue,size,0);
    
    int* C= (int*)malloc(sizeof(int) * size);
    vecAddCopyOutputDeviceArrayToHostArray(C,buffer_C,queue,size,0);
...
void vecAddRunAKernelOnTwoDevicesSecondVersion(cl_device_type type, int size, double prozent){
    ...
    //1.on Host
    int offset1=0;
    int sizeOfFirstDevice=(int)size*prozent;
...
void vecAdd(int size){
    ...
    double prozent=0.1;
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        vecAddRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
...
void bitCompressionRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    bitCompressionCopyInputHostArrayToDeviceArray(input,num_bits,buffer_A,buffer_B,queue,size,0);
    
    cl::Kernel bit_compression=cl::Kernel(program,"bit_compression");
    bitCompressionRunTheKernel(bit_compression,program,buffer_A,buffer_B,buffer_C,queue,size,0);
    
    cl_uint* output = (cl_uint*) malloc(sizeof(cl_uint) * size);
    bitCompressionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue,size,0);
    
    bitCompressionPrintTheResult(input,num_bits,output,size);
...
void bitCompressionRunAKernelOnTwoDevices(cl_device_type type, int size){
    ...
    bitCompressionCopyInputHostArrayToDeviceArray(input1,num_bits1,buffer_A1,buffer_B1,queue1,halfOfTheSize,0);
     bitCompressionCopyInputHostArrayToDeviceArray(input2,num_bits2,buffer_A2,buffer_B2,queue2,halfOfTheSize,0);
    ...
    bitCompressionRunTheKernel(bit_compression_1,program1,buffer_A1,buffer_B1,buffer_C1,queue1,halfOfTheSize,0);
    bitCompressionRunTheKernel(bit_compression_2,program2,buffer_A2,buffer_B2,buffer_C2,queue2,halfOfTheSize,0);
    
    cl_uint* output1 = (cl_uint*) malloc(sizeof(cl_uint) * halfOfTheSize);
    cl_uint* output2 = (cl_uint*) malloc(sizeof(cl_uint) * halfOfTheSize);
    
    bitCompressionCopyOutputDeviceArrayToHostArray(output1,buffer_C1,queue1,halfOfTheSize,0);
    bitCompressionCopyOutputDeviceArrayToHostArray(output2,buffer_C2,queue2,halfOfTheSize,0);
    

    bitCompressionPrintTheResultForTwoDevices(input1,input2,num_bits1,num_bits2,
                                              output1,output2,halfOfTheSize);
...
void bitCompressionRunAKernelOnTwoDevicesSecondVersion(cl_device_type type,
                                                       int size,double prozent){
    std::vector<cl::Platform> all_platforms;
    cl::Platform default_platform=getADefaultPlatform(all_platforms);
    ...
    bitCompressionCopyInputHostArrayToDeviceArray(input,num_bits,buffer_A,buffer_B,queue1,size,offset1);
    bitCompressionCopyInputHostArrayToDeviceArray(input,num_bits,buffer_A,buffer_B,queue2,size,offset2);
    
    cl::Kernel bit_compression_1=cl::Kernel(program1,"bit_compression");
    cl::Kernel bit_compression_2=cl::Kernel(program2,"bit_compression");
    
    bitCompressionRunTheKernel(bit_compression_1,program1,buffer_A,buffer_B,buffer_C,queue1,size,offset1);
    bitCompressionRunTheKernel(bit_compression_2,program2,buffer_A,buffer_B,buffer_C,queue2,size,offset2);
    
    cl_uint* output = (cl_uint*) malloc(sizeof(cl_uint) * size);
    
    bitCompressionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue1,size,offset1);
    bitCompressionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue2,size,offset2);
...
void bitCompression(int size){
    ...
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        bitCompressionRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
...
void matMulRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    matMulCopyInputHostArrayToDeviceArray(input1,input2,buffer_A,buffer_B,queue,sizeOfTheMatrix,0);
    
    cl::Kernel mat_mul=cl::Kernel(program,"mat_mul");
    matMulRunTheKernel(mat_mul,program,buffer_A,buffer_B,buffer_C,queue,sizeOfTheMatrix,width,0);
    
    int* output = (int *)malloc(sizeof(int) * size);
    matMulCopyOutputDeviceArrayToHostArray(output,buffer_C,queue,sizeOfTheMatrix,0);
...
void matMulRunAKernelOnTwoDevices(cl_device_type type, int size){
    ...
    int newWidth = width;
    int halfSizeOfTheMatrix = (int)floor(size/2);
    ...
    matMulCopyInputHostArrayToDeviceArray(input1device1,input2device1,buffer_A1,buffer_B1,queue1,halfSizeOfTheMatrix,0);
    matMulCopyInputHostArrayToDeviceArray(input1device2,input2device2,buffer_A2,buffer_B2,queue2,halfSizeOfTheMatrix,0);
    
    cl::Kernel mat_mul_1=cl::Kernel(program1,"mat_mul");
    cl::Kernel mat_mul_2=cl::Kernel(program2,"mat_mul");
    
    matMulRunTheKernel(mat_mul_1,program1,buffer_A1,buffer_B1,buffer_C1,queue1,halfSizeOfTheMatrix,newWidth,0);
    matMulRunTheKernel(mat_mul_2,program2,buffer_A2,buffer_B2,buffer_C2,queue2,halfSizeOfTheMatrix,newWidth,0);
    
    int* output1 = (int *)malloc(sizeof(int) * halfSizeOfTheMatrix);
    int* output2 = (int *)malloc(sizeof(int) * halfSizeOfTheMatrix);
    
    matMulCopyOutputDeviceArrayToHostArray(output1,buffer_C1,queue1,halfSizeOfTheMatrix,0);
    matMulCopyOutputDeviceArrayToHostArray(output2,buffer_C2,queue2,halfSizeOfTheMatrix,0);
...
void matMulRunAKernelOnTwoDevicesSecondVersion(cl_device_type type,int size,double prozent){
    std::vector<cl::Platform> all_platforms;
    cl::Platform default_platform=getADefaultPlatform(all_platforms);
    ...
    matMulCopyInputHostArrayToDeviceArray(input1,input2,buffer_A,buffer_B,queue1,sizeOfTheMatrix,offset1);
    matMulCopyInputHostArrayToDeviceArray(input1,input2,buffer_A,buffer_B,queue2,sizeOfTheMatrix,offset2);
    
    cl::Kernel mat_mul_1=cl::Kernel(program1,"mat_mul");
    cl::Kernel mat_mul_2=cl::Kernel(program2,"mat_mul");
    
    matMulRunTheKernel(mat_mul_1,program1,buffer_A,buffer_B,buffer_C,queue1,sizeOfTheMatrix,width,offset1);
    matMulRunTheKernel(mat_mul_1,program2,buffer_A,buffer_B,buffer_C,queue2,sizeOfTheMatrix,width,offset2);
    
    int* output = (int *)malloc(sizeof(int) * sizeOfTheMatrix);

    matMulCopyOutputDeviceArrayToHostArray(output,buffer_C,queue1,sizeOfTheMatrix,offset1);
    matMulCopyOutputDeviceArrayToHostArray(output,buffer_C,queue2,sizeOfTheMatrix,offset2);
...
void matMul(int size){
    ...
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        matMulRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
...
void linRegRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    linRegCopyInputHostArrayToDeviceArray(input1,input2,alpha,beta,buffer_A,buffer_B,buffer_C,buffer_D,queue,size,0);
    
    cl::Kernel lin_reg=cl::Kernel(program,"lin_reg");
    linRegRunTheKernel(lin_reg,program,buffer_A,buffer_B,buffer_C,buffer_D,buffer_E,queue,size,0);
    
    float* output = (float*) malloc(sizeof(float) * size);
    linRegCopyOutputDeviceArrayToHostArray(output,buffer_E,queue,size,0);
...
void linRegRunAKernelOnTwoDevices(cl_device_type type, int size){
    ...
    linRegCopyInputHostArrayToDeviceArray(input1device1,input2device1,alpha1,beta1,buffer_A1,buffer_B1,buffer_C1,buffer_D1,queue1,halfOfTheSize,0);
    linRegCopyInputHostArrayToDeviceArray(input1device2,input2device2,alpha2,beta2,buffer_A2,buffer_B2,buffer_C2,buffer_D2,queue2,halfOfTheSize,0);
    
    cl::Kernel lin_reg_1=cl::Kernel(program1,"lin_reg");
    cl::Kernel lin_reg_2=cl::Kernel(program2,"lin_reg");
    linRegRunTheKernel(lin_reg_1,program1,buffer_A1,buffer_B1,buffer_C1,buffer_D1,buffer_E1,queue1,halfOfTheSize,0);
    linRegRunTheKernel(lin_reg_2,program2,buffer_A2,buffer_B2,buffer_C2,buffer_D2,buffer_E2,queue2,halfOfTheSize,0);
    
    float* output1 = (float*) malloc(sizeof(float) * halfOfTheSize);
    float* output2 = (float*) malloc(sizeof(float) * halfOfTheSize);
    linRegCopyOutputDeviceArrayToHostArray(output1,buffer_E1,queue1,halfOfTheSize,0);
    linRegCopyOutputDeviceArrayToHostArray(output2,buffer_E2,queue2,halfOfTheSize,0);
...
void linRegRunAKernelOnTwoDevicesSecondVersion(cl_device_type type,int size,double prozent){
    std::vector<cl::Platform> all_platforms;
    cl::Platform default_platform=getADefaultPlatform(all_platforms);
    ...
    linRegCopyInputHostArrayToDeviceArray(input1,input2,alpha,beta,buffer_A,buffer_B,buffer_C,buffer_D,queue1,size,offset1);
    linRegCopyInputHostArrayToDeviceArray(input1,input2,alpha,beta,buffer_A,buffer_B,buffer_C,buffer_D,queue2,size,offset2);
    
    cl::Kernel lin_reg_device1=cl::Kernel(program1,"lin_reg");
    cl::Kernel lin_reg_device2=cl::Kernel(program2,"lin_reg");
    
    linRegRunTheKernel(lin_reg_device1,program1,buffer_A,buffer_B,buffer_C,buffer_D,buffer_E,queue1,size,offset1);
    linRegRunTheKernel(lin_reg_device2,program2,buffer_A,buffer_B,buffer_C,buffer_D,buffer_E,queue2,size,offset2);
    
    float* output = (float*) malloc(sizeof(float) * size);
    linRegCopyOutputDeviceArrayToHostArray(output,buffer_E,queue1,size,offset1);
    linRegCopyOutputDeviceArrayToHostArray(output,buffer_E,queue2,size,offset2);
...
void linReg(int size){
    ...
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        linRegRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
...
void syr2kRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    syr2kCopyInputHostArrayToDeviceArray(input_a,input_b,output,buffer_A,buffer_B,buffer_C,queue,newSize,0);
    
    cl::Kernel syr2k1=cl::Kernel(program,"syr2k");
    syr2kRunTheKernel(syr2k1,program,buffer_A,buffer_B,buffer_C,ALPHA,BETA,M,N,queue,newSize,0);
    
    syr2kCopyOutputDeviceArrayToHostArray(output,buffer_C,queue,newSize,0);
...
//TODO: only works for size 12..why???
void syr2kRunAKernelOnTwoDevicesSecondVersion(cl_device_type type,int size,double prozent){
    std::vector<cl::Platform> all_platforms;
    cl::Platform default_platform=getADefaultPlatform(all_platforms);
    ...
    syr2kCopyInputHostArrayToDeviceArray(input_a,input_b,output,buffer_A,buffer_B,buffer_C,queue1,newSize,offset1);
    syr2kCopyInputHostArrayToDeviceArray(input_a,input_b,output,buffer_A,buffer_B,buffer_C,queue2,newSize,offset2);
    
    cl::Kernel syr2k1_device1=cl::Kernel(program1,"syr2k");
    cl::Kernel syr2k1_device2=cl::Kernel(program2,"syr2k");
    
    syr2kRunTheKernel(syr2k1_device1,program1,buffer_A,buffer_B,buffer_C,ALPHA,BETA,M,N,queue1,newSize,offset1);
    syr2kRunTheKernel(syr2k1_device2,program2,buffer_A,buffer_B,buffer_C,ALPHA,BETA,M,N,queue2,newSize,offset2);
    
    syr2kCopyOutputDeviceArrayToHostArray(output,buffer_C,queue1,newSize,offset1);
    syr2kCopyOutputDeviceArrayToHostArray(output,buffer_C,queue2,newSize,offset2);
...
void syr2k(int size){
    ...
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        syr2kRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
...
void convolutionRunAKernelOnOneDevice(cl_device_type type, int size){
    ...
    convolutionCopyInputHostArrayToDeviceArray(input,mask,output,buffer_A,buffer_B,buffer_C,queue,newSize,mask_size,0);
    
    cl::Kernel convolution1=cl::Kernel(program,"convolution");
    convolutionRunTheKernel(convolution1,program,buffer_A,buffer_B,buffer_C,width,mask_width,queue,newSize,0);
    
    convolutionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue,newSize,0);
...
void convolutionRunAKernelOnTwoDevicesSecondVersion(cl_device_type type,int size,double prozent){
    std::vector<cl::Platform> all_platforms;
    cl::Platform default_platform=getADefaultPlatform(all_platforms);
    ...
    convolutionCopyInputHostArrayToDeviceArray(input,mask,output,buffer_A,buffer_B,buffer_C,queue1,newSize,mask_size,offset1);
    convolutionCopyInputHostArrayToDeviceArray(input,mask,output,buffer_A,buffer_B,buffer_C,queue2,newSize,mask_size,offset2);
    
    cl::Kernel convolution_device1=cl::Kernel(program1,"convolution");
    cl::Kernel convolution_device2=cl::Kernel(program2,"convolution");
    
    convolutionRunTheKernel(convolution_device1,program1,buffer_A,buffer_B,buffer_C,width,mask_width,queue1,newSize,offset1);
    convolutionRunTheKernel(convolution_device2,program2,buffer_A,buffer_B,buffer_C,width,mask_width,queue2,newSize,offset2);
    
    convolutionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue1,newSize,offset1);
    convolutionCopyOutputDeviceArrayToHostArray(output,buffer_C,queue2,newSize,offset2);
...
void convolution(int size){
    ...
    for (int i=1; i<10; i++) {
        std::cout<<"Prozent: "<< prozent <<endl;
        convolutionRunAKernelOnTwoDevicesSecondVersion(CL_DEVICE_TYPE_ALL,size,prozent);
        prozent=prozent+0.1;
        std::cout<<" \n\n";
    }
}
